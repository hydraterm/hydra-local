// Provider metadata and label inference shared by the browser engine; artwork belongs to the hosted UI adapter.
...
export type RemoteAgentKind = RunnableAgentKind | 'terminal'
...
export const COPILOT_MODELS = [
  'default', 'auto', 'claude-sonnet-5', 'claude-sonnet-4.6', 'claude-sonnet-4.5',
  'claude-haiku-4.5', 'claude-fable-5', 'claude-opus-4.8', 'claude-opus-4.8-fast',
  'claude-opus-4.7', 'claude-opus-4.6', 'claude-opus-4.5', 'gpt-5.6-sol',
...
export const ANTIGRAVITY_MODELS = [
  'default', 'Gemini 3.5 Flash (Medium)', 'Gemini 3.5 Flash (High)',
...
// catalog can add newer IDs without requiring a browser release.
export const CURSOR_MODELS = [
  'default', 'auto', 'composer-2.5', 'gpt-5.3-codex', 'gpt-5.3-codex-high',
  'gpt-5.3-codex-xhigh', 'claude-sonnet-5-high', 'claude-opus-4-8-high',
  'gpt-5.6-sol-high', 'gpt-5.5-high', 'gpt-5.4-high', 'gemini-3.1-pro',
  'kimi-k2.7-code', 'glm-5.2-high',
] as const
...
export const AGENT_PROVIDERS: Record<RemoteAgentKind, AgentProvider> = {
  claude: {
  ...
  kimi: {
    command: 'kimi', label: 'Kimi', glyph: '☾', color: '#f0b35b', dangerousFlag: '--yolo',
    selectable: true,
  ...
  terminal: {
    command: 'bash', label: 'Terminal', glyph: '›', color: '#9aa4b2', dangerousFlag: '',
...
] as const satisfies readonly RemoteAgentKind[]
...
// Amp and Factory stay out of history queries until their provider-owned stores are qualified.
...
export function isRemoteAgentKind(value: unknown): value is RemoteAgentKind {
...
export function supportsPermanentHistoryDelete(agent: RemoteAgentKind): boolean {
  return agent !== 'copilot' && agent !== 'antigravity' && agent !== 'kimi' && agent !== 'kiro'
    && agent !== 'cursor' && agent !== 'amp' && agent !== 'devin' && agent !== 'factory'
}
...
export function buildAgentLaunchArgv(options: AgentLaunchOptions): string[] {
  ...
  if (agent === 'terminal') return parts
...
/**
 * Derive the agent from a session label — ONLY when the label clearly names a known agent (a whole-word match,
 * case-insensitive). Returns null otherwise. Honest: we never guess; a label that doesn't name an agent gets no
 * badge rather than a fabricated one. ('shell' is intentionally NOT inferred from a label — it's the desktop's
 * explicit no-agent default, which the browser can't honestly assert from a label alone.)
 */
export function agentFromLabel(label: string | undefined): AgentKind | null {
  if (!label) return null
  const lower = label.toLowerCase()
  for (const agent of RUNNABLE_AGENT_OPTIONS) {
    // AMP and Factory are launchable but deliberately have no label inference. Their provider IDs do not
    // establish executable provenance from a custom title, so never turn such a title into inherited launch
    // authority for a later split/create operation.
    if (agent === 'amp' || agent === 'factory') continue
    // whole-word match so e.g. "codex-notes" matches but "encodex" / "claudette" do not.
    if (new RegExp(`\\b${agent}\\b`).test(lower)) return agent
  }
  return null
}
